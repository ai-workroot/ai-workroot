"""Compatibility managed-state initialization for Clean Workroot flows."""
...
@contextmanager
def registry_lock(home: Path, timeout: float = 10.0) -> Iterator[Path]:
    ...
    if fcntl is None:
        ...
        while True:
            try:
                token_file = token_path.open("x", encoding="utf-8")
                break
        ...
        try:
            with token_file:
                token_file.write(str(time.time()))
            yield lock_path
        finally:

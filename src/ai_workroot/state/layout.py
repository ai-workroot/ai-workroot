...
class CleanModeBoundaryError(ValueError):
    """Raised when managed state would be placed inside user content."""
...
def workroot_state_dir(ai_workroot_home: Path, workroot_id: str) -> Path:
    ...
    try:
        ...
        raise CleanModeBoundaryError(
            f"Workroot state directory escaped AI_WORKROOT_HOME/workroots: {resolved_state_directory}"
        ) from exc
...
def validate_user_directory(user_directory: Path, ai_workroot_home: Path, create: bool = True) -> Path:
    ...
    if platform.system() != "Windows":
        obvious_system_dirs.update(
            Path(path).resolve() for path in ("/bin", "/sbin", "/usr", "/etc", "/var", "/System", "/Library")
        )

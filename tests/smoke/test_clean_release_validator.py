...
class CleanReleaseValidatorSmokeTest(unittest.TestCase):
    ...
    def test_release_doctor_fails_tracked_public_seed_root_files(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (repo / "src/ai_workroot/entrypoints/native_agent/templates/AGENTS.md.template").write_text(
                "", encoding="utf-8"
            )
    ...
    def test_export_review_zip_excludes_ignored_local_artifacts(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (repo / "src/ai_workroot/capabilities/context").mkdir(parents=True)
            (repo / "src/ai_workroot/capabilities/context/builder.py").write_text("# tracked\n", encoding="utf-8")
            ...
            self.assertIn("src/ai_workroot/capabilities/context/builder.py", names)

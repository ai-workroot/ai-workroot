...
def _require_runner_roots(testcase: unittest.TestCase, suite_name: str) -> tuple[Path, Path]:
    require_e2e_runner_active(testcase, suite_name)
    roots = _runner_roots()
    if roots is None:
        testcase.skipTest(f"{suite_name} E2E requires runner roots")
    return roots


class LongrunE2ETest(unittest.TestCase):
    def test_longrun_keeps_process_fixtures_out_of_user_directories(self) -> None:
        run_root, sandbox_base = _require_runner_roots(self, "longrun")
        result = run_longrun(
            run_root=run_root,
...
class ActiveContextRuntimeLongrunTest(unittest.TestCase):
    def test_debug_context_keeps_trace_lines_when_hard_trim_runs(self) -> None:
        run_root, sandbox_base = _require_runner_roots(self, "longrun")
        smoke = run_persona_smoke(run_root=run_root, sandbox_base=sandbox_base)

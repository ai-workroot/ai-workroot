#!/usr/bin/env python3
"""Archived 0.9.529 script migration runner before package migration."""
...
try:
    from workroot_client import now_utc
except ModuleNotFoundError:  # pragma: no cover - package import path for tests.
    from scripts.workroot_client import now_utc
...
@contextmanager
def migration_lock(root: Path, scope: str, timeout: float = 10.0):
    ...
    while True:
        try:
            ...
            if time.monotonic() - start > timeout:
                raise SystemExit(f"timed out waiting for migration lock: {lock_path}")
            time.sleep(0.02)
    try:
        yield
    finally:

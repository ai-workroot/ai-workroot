#!/usr/bin/env python3
"""Archived 0.9.529 Materialized Context Candidate repository before package migration."""
...
def query_context_candidates(
    ...
    if not include_blocked_safety:
        filters.append(
            "(safety_policy IS NULL OR safety_policy = '' OR safety_policy NOT IN ({}))".format(
                ",".join("?" for _ in BLOCKED_SAFETY_POLICIES)
            )
        )
